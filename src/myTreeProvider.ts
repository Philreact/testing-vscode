...
`,
    ...
    this.contextValue = this.data.hasDownloadableContent
      ? 'archiveWithFiles'
      : 'archive';
  }
}
...
class FilesNode extends TreeItem {
  constructor(label: string, public readonly files: FilesContainer) {
...
function _filteredData(filter: Filter): Archive[] {
  ...
  for (const item of DATA) {
    if (filter.term && filter.hasFiles) {
      if (
        (item.title.includes(filter.term) ||
...
export class TestTreeDataProvider
  implements TreeDataProvider<DomainObject>, Disposable
{
  public static readonly VIEW = 'extension.myTreeView';
  public static readonly CONTEXT_HAS_FILTER =
    TestTreeDataProvider.VIEW + '.hasFilter';
  public static readonly NEXT_PAGE_COMMAND =
    TestTreeDataProvider.VIEW + '.loadNextPage';
  public static readonly RESET_COMMAND =
    TestTreeDataProvider.VIEW + '.resetFilter';
  public static readonly DELETE_COMMAND = TestTreeDataProvider.VIEW + '.delete';
  public static readonly DOWNLOAD_SINGLE_FILE_COMMAND =
    TestTreeDataProvider.VIEW + '.downloadFile';
  public static readonly DOWNLOAD_MOST_RECENT_COMMAND =
    TestTreeDataProvider.VIEW + '.downloadMostRecent';

  private _onDidChangeTreeData: EventEmitter<
    void | DomainObject | null | undefined
  > = new EventEmitter<void | DomainObject | null | undefined>();
  onDidChangeTreeData?:
    | Event<void | DomainObject | null | undefined>
  ...
  constructor(context: ExtensionContext) {
    this.disposables.push(
      window.registerTreeDataProvider(TestTreeDataProvider.VIEW, this),
    );

    this.disposables.push(
      commands.registerCommand(
        TestTreeDataProvider.RESET_COMMAND,
        this.resetFilter.bind(this),
      ),
    ); // or () => this.resetFilter()
    this.disposables.push(
      commands.registerCommand(
        TestTreeDataProvider.NEXT_PAGE_COMMAND,
        this.nextPage.bind(this),
      ),
    ); // or (page: number) => this.nextPage(page)
    this.disposables.push(
      commands.registerCommand(
        TestTreeDataProvider.DOWNLOAD_MOST_RECENT_COMMAND,
        this.downloadMostRecent.bind(this),
      ),
    );
    this.disposables.push(
      commands.registerCommand(
        TestTreeDataProvider.DOWNLOAD_SINGLE_FILE_COMMAND,
        this.downloadFile,
      ),
    );
    this.disposables.push(
      commands.registerCommand(
        TestTreeDataProvider.DELETE_COMMAND,
  ...
  private nextPage(page: number): void {
    if (
      this.items.length > 0 &&
      this.items[this.items.length - 1] instanceof LoadMore
    ) {
  ...
  public async delete(item: Archive): Promise<void> {
    const choice = await this.promptUserAction(
      `Do you really want to delete archive ${item.title}?`,
      'Yes',
      'No',
    );
    if (choice === 'Yes') {
      ...
      ); // and that the UUID matches the one provided as input
  ...
  private async promptUserAction(
    ...
    return new Promise<string | undefined>((resolve, reject) => {
      void window
        .showWarningMessage(msg, { modal: true }, ...choices)
  ...
  public async updateFilter(filter: Filter): Promise<void> {
    ...
    await commands.executeCommand(
      'setContext',
      TestTreeDataProvider.CONTEXT_HAS_FILTER,
      true,
    );
    this.items = [];
    this._onDidChangeTreeData.fire();
